...
Elk besluit landt in YAML in `data/` en wordt bij elke volgende verwerking opnieuw toegepast: liedregels als alias (genormaliseerde tekst naar lied-id), Bijzonderheden onder een hash van de tekst (zodat namen niet in de publieke data komen), kandidaten als aanvulling per dienst, nieuwe liederen in de catalogus. Een onbekend bundelnummer met een titel die sterk lijkt op een bestaand lied wordt niet stilzwijgend samengevoegd; dat ging mis bij Sela "Hoe machtig is uw naam" en een Opwekking Kids-lied met dezelfde titel. Het item krijgt een twijfel-regel en de beheerder kiest: hetzelfde lied in een andere bundel (het nummer wordt dan als verwijzing toegevoegd) of een ander lied met dezelfde titel.
...
**Fase 2, website.** Gebouwd, na twee wireframe-rondes. Publicatiestap `publiceer` maakt `site/data/dataset.json` (diensten met lied-id en moment, plus de gebruikte liederen met referenties en categorieën, geen ruwe tekst behalve bij niet-herkende regels) en een CSV. De website is platte HTML en JavaScript zonder bouwstap, met hash-routes: dashboard, diensten, lied, over. Dashboard: filterbalk bovenaan die voor alle blokken geldt (categorie, begeleiding, bundel, zoekveld), een hoog blok "recent gezongen" gegroepeerd per dienst met per lied hoe vaak het al voorkwam, en daarnaast "tijd voor een tweede keer", "vaak gezongen" en "vergeten bekenden", elk met het criterium instelbaar in de kopregel en per persoon bewaard in de browser. Statistieken staan op de pagina "Over", samen met de uitleg van de criteria en de CSV-link. De voettekst van elke pagina noemt de begindatum van de gegevens, feitelijk en zonder excuus: "voor het eerst" en "nooit" zijn altijd sinds die datum. Lieddetail met maandtijdlijn, alle datums en zoeklinks naar Kerkliedwiki, YouTube en Spotify. GitHub Actions bouwt de dataset bij elke push en publiceert `site/` naar GitHub Pages. Nog te doen in een volgende ronde: de liederenlijst (bladeren op categorie of gelegenheid, met "op doopzondag gezongen" gemengd tussen de getagde liederen) en seizoensblik.
